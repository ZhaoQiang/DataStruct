...
typedef Tsd::CLFFifo< uint32_t * > TestQ;
...
void test2()
{
  ...
  Tsd::CLFFifo< uint32_t > tQ(100);

  TEST_FOR( tQ.size() == 0 );
  TEST_FOR( tQ.capacity() == 100 );
  ...
  for( i = 0; i < 100; i++ )
    {
      ...
      TEST_FOR( tItem == i );
      TEST_FOR( tQ.size() == (100 - (i+1)) );
    }

  // The underflow flag should be set when the last item is removed..
  TEST_FOR( tQ.test_underflow() );

  // once cleared, it should not be set again until something has been in
  // the queue. i.e. it only sets on the transition from size = 1 to size = 0
  TEST_FOR( tQ.tac_underflow() );
  TEST_FOR( !tQ.test_underflow() );
  ...
  TEST_FOR( tQ.insert( 100 ) );
  TEST_FOR( tQ.size() == 1 );
  TEST_FOR( !tQ.tac_underflow() );
  TEST_FOR( tQ.remove( i ) );
  TEST_FOR( i == 100 );
  TEST_FOR( tQ.tac_underflow() );
  TEST_FOR( !tQ.tac_underflow() );
